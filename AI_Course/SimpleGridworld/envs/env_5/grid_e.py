...
for epsilon in ep_y['y']:
    pygame.display.set_caption(f"epsilon={epsilon}")
    act = 1
    eps_start = epsilon
    converged = False
    failed = False
    ...
    while not converged:
        ...
        while not over:
            ...
            if not over:
                ...
                if actions > LEN * LEN * 4 and epsilon < 0.2:
                    over = True
                    converged = True
                    failed = True
                    print("failed")
                ...
                if reward == env.target_reward:
                    if actions < best_action:
                        best_action_counter = 00
                        best_action = actions
                        print(f"{act} BEST episode with {best_action} eps={epsilon}")
        ...
        epsilon = np.max(epsilon - epsilon_decay_val, 0)
    if failed:
        ep_y['ep'].append(act)
        ep_failed.append(f"F-{eps_start}")
    elif not failed:
        ep_y['ep'].append(act - 40)
        ep_failed.append(f"A-{eps_start}")
